        ...
        if ($attempts['count'] >= 5 && (time() - $attempts['time']) < 300) {
            ...
            elseif (!preg_match('/^09[0-9]{9}$/', $phone)) { $errors[] = 'شماره موبایل نامعتبر است.'; }
                ...
                if ($user && password_verify($password, $user['password_hash'])) {
                    ...
                    setcookie('golestan_user', $user['id'], ['expires' => time() + (86400 * 30), 'path' => '/', 'secure' => true, 'httponly' => true, 'samesite' => 'Lax']);
                    ...
                    if ((time() - $attempts['time']) > 300) { $attempts = ['count' => 1, 'time' => time()]; }
                    else { $attempts['count']++; }
...
<div class="auth-container"><div class="auth-box"><div class="auth-header"><div class="auth-icon"><i class="fas fa-sign-in-alt"></i></div><h1>ورود به <?php echo SITE_NAME; ?></h1><p>از طریق گوگل، گیت‌هاب یا شماره موبایل وارد شوید</p></div>
<?php if (!empty($errors)): ?><div class="alert alert-error"><?php foreach ($errors as $e) echo "<p>$e</p>"; ?></div><?php endif; ?>
<a href="/oauth/google-login.php" class="oauth-btn"><img src="https://www.google.com/favicon.ico" alt="Google" width="20" height="20"> ورود با حساب گوگل</a>
<a href="/oauth/github-login.php" class="oauth-btn github-btn"><i class="fab fa-github"></i> ورود با گیت‌هاب</a>
...
<div class="form-group"><label>📱 شماره موبایل</label><input type="tel" name="phone" value="<?php echo $phone; ?>" placeholder="09xxxxxxxxx" required></div>
<div class="form-group"><label>🔒 رمز عبور</label><input type="password" name="password" placeholder="رمز عبور" required></div>
<div class="form-options"><label class="remember-me"><input type="checkbox"> مرا به خاطر بسپار</label><a href="/forgot-password.php" class="forgot-link">فراموشی رمز؟</a></div>
<button type="submit" class="btn btn-primary btn-block">ورود به حساب</button></form>
<div class="auth-footer"><p>حساب ندارید؟ <a href="/signup.php">ثبت‌نام کنید و ۱۰۰۰ اعتبار بگیرید</a></p><p><a href="/">← بازگشت به صفحه اصلی</a></p></div></div></div>
