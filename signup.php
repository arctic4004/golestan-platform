...
function isLoggedInSignup()
{
    return isset($_SESSION['user_id']) && !empty($_SESSION['user_id']);
}
function sanitizeSignup($data)
{
        ...
        if ($attempts['count'] >= 3 && (time() - $attempts['time']) < 600) {
            ...
            if (empty($errors)) {
                ...
                if (!empty($email)) {
                    ...
                    $stmt->execute([$email]);
                    if ($stmt->fetch()) $errors[] = 'این ایمیل قبلاً ثبت شده است.';
                }
            }
            if (empty($errors)) {
                ...
                header("Location: /user/dashboard/v2/?welcome=1");
                exit();
            }
        }
    }
...
<div class="auth-container">
    <div class="auth-box">
        ...
        <form method="POST"><input type="hidden" name="csrf_token" value="<?php echo $_SESSION['csrf_token']; ?>">
            ...
            <button type="submit" class="btn btn-primary btn-block"><i class="fas fa-user-plus"></i> ثبت‌نام و دریافت ۱۰۰۰ اعتبار</button>
        </form>
